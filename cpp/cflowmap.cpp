...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
    ...
    invalidValue = mxGetScalar(prhs[6]);
    out_res = *(Vec3i*)mxGetPr(prhs[7]);
    slice = nrhs>=9 ? (int)mxGetScalar(prhs[8]) : 0;
    
    /* Output arguments */
    plhs[0] = mxCreateNumericMatrix(out_res[0], out_res[1] * (nrhs>=9 ? 1 : out_res[2]) * 2, mxSINGLE_CLASS, mxREAL);
    result = (Vec2f*) mxGetData(plhs[0]);

    /* Perform the computation */
    UnsteadyVectorField2d inField(resolution, BoundingBox3d(minDomain, maxDomain), data);
    if (nrhs>=9) {
        SteadyVectorField2d outField(Vec2i({ out_res[0], out_res[1] }), BoundingBox2d(Vec2d({ minDomain[0], minDomain[1] }), Vec2d({ maxDomain[0], maxDomain[1] })), result);
        FlowMap2d::Compute(inField, stepSize, duration, invalidValue, outField, slice);
    }
    else {
        UnsteadyVectorField2d outField(out_res, BoundingBox3d(minDomain, maxDomain), result);
        FlowMap2d::Compute(inField, stepSize, duration, invalidValue, outField);	
    }
}
